...
bool FileHeader::Allocate(PersistentBitmap *freeMap, int fileSize)
{
	...
	if(fileSize > MaxFileSize){
		...
		bool success = nextFileHeader->Allocate(freeMap, fileSize-numBytes);
		if (success) nextFileHeader->WriteBack(nextFileHeaderSector);
		delete nextFileHeader;
		return success;
	}
	// DEBUG('f', "Allocate len")
...
int FileHeader::ByteToSector(int offset) //virtual -> physical 找到sector num
{
	...
	DEBUG('f', "Byte to sector, offset: " << offset);
	//MP4-2
	if(offset > numSectors) {
		FileHeader* nextFileHeader = new FileHeader;
		nextFileHeader->FetchFrom(nextFileHeaderSector);
		int sec = nextFileHeader->ByteToSector(offset - numSectors);
		DEBUG('f', "Byte to sector, sector: " << sec);
		delete nextFileHeader;
		return sec;
	}
...
int FileHeader::FileLength()
{
	...
	if(nextFileHeaderSector == -1) return numBytes;
		...
		FileHeader* nextFileHeader = new FileHeader;
		nextFileHeader->FetchFrom(nextFileHeaderSector);
		int len = numBytes + nextFileHeader->FileLength();
		delete nextFileHeader;
		return len;
	}
}

...
void FileHeader::WriteBack(int sector)
{
	...
	if(nextFileHeader != NULL) nextFileHeader->WriteBack(nextFileHeaderSector); //MP4-2
...
int FileHeader::ByteToSector(int offset)
{
    ...
    if (sector >= NumDirect) return nextFileHeader->ByteToSector(offset - MaxFileSize);
...
int FileHeader::FileLength()
{
	...
	if(nextFileHeader == NULL) return numBytes;
